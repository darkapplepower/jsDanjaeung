...
function onCreate(savedInstanceState, activity) {
    const {x,y}=getSize(activity);
    var mainLayout=new LinearLayout(activity);
    
    var scrollView=new ScrollView(activity);
    var layoutp = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
    layoutp.width=x;
    scrollView.setLayoutParams(layoutp);
    
    var finalMainLayout=new LinearLayout(activity);
    finalMainLayout.setOrientation(1);
    
    var title=new TextView(activity);
    title.setText("메신저봇용 단자응");
    title.setGravity(Gravity.CENTER | Gravity.TOP);
    title.setTextSize(x/30);
    
    var spinner=new Spinner(activity);
    spinner.setAdapter(new android.widget.ArrayAdapter(activity,android.R.layout.simple_list_item_1,["일치","시작","포함"]));
    
    var input1Layout = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
    input1Layout.width=x;
    
    var input1=new EditText(activity);
    input1.setHint("이렇게 말하면...");
    input1.setLayoutParams(input1Layout);
    
    var input2Layout = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
    input2Layout.setMargins(0,y/25,0,0);
    input2Layout.width=x;
    
    var input2=new EditText(activity);
    input2.setHint("이렇게 답하기");
    input2.setLayoutParams(input2Layout);
    ...
    {
        onClick:function(){
            plusDanjaeung(input1, input2, spinner);
            resetlist();
        }
    });
    function resetlist(){
        list.removeAllViews();
        setLayout(list, activity, x, y);
    }
    
    const info=new LinearLayout(activity);
    
    const infoText1=new TextView(activity);
    infoText1.setText("이렇게 말하면");
    infoText1.setWidth((x/10)*4);
    
    const infoText2=new TextView(activity);
    infoText2.setText("이렇게 답하기");
    infoText2.setWidth((x/10)*4);
    
    const infoText3=new TextView(activity);
    infoText3.setText("삭제버튼");
    infoText3.setWidth((x/10)*2);
    
    info.addView(infoText1);
    info.addView(infoText2);
    info.addView(infoText3);
    
    
    var list=new LinearLayout(activity);
    list.setOrientation(1);
    resetlist();
    
    finalMainLayout.addView(title);
    finalMainLayout.addView(spinner);
    finalMainLayout.addView(input1);
    finalMainLayout.addView(input2);
    finalMainLayout.addView(button);
    finalMainLayout.addView(info);
    finalMainLayout.addView(list);
    
    scrollView.addView(finalMainLayout);
    
    mainLayout.addView(scrollView);
    
    activity.setContentView(mainLayout);
}
function setLayout(layout, activity, x, y){
    for(let i=0;i<danjaList.length;i++){
        layout.addView(getList(danjaList[i], activity, x, y, layout));
    }
}
function getList(num, activity, x, y, layout){
    const result=new LinearLayout(activity);
    
    
    const textView1=new TextView(activity);
    textView1.setText(num.msg);
    textView1.setTextColor(modeColor[num.mode]);
    textView1.setWidth((x/10)*4);
    
    const textView2=new TextView(activity);
    textView2.setText(num.reply);
    textView2.setWidth((x/10)*4);
    
    const button=new Button(activity);
    button.setText("X");
    button.setWidth((x/10)*2);
    button.setOnClickListener(new View.OnClickListener()
    {
        onClick:function(){
            danjaList.splice(danjaList.indexOf(num),1);
            saveBlock();
    ...
    result.addView(textView2);
    result.addView(button);
    return result;
}
const danjaList=JSON.parse(Array.isArray(JSON.parse(FileStream.read(savePath+"data.json")))?FileStream.read(savePath+"data.json"):FileStream.write(savePath+"data.json","[]"));
function plusDanjaeung(input1, input2, spinner1){
    danjaList.push(new Block(input1.getText(), input2.getText(), spinner1.getSelectedItemPosition()));
    input1.setText("");
    input2.setText("");
    saveBlock();
}
...
function saveBlock(){
    FileStream.write(savePath+"data.json",JSON.stringify(danjaList));
}
